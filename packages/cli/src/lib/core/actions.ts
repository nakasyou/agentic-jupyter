...
function defineAction(name: string, description: string, inputSchema: z.ZodTypeAny): ActionDefinition {
  return {
    name,
...
export const actionDefinitions = [
  defineAction(
    ...
    'Connect directly to a reachable Jupyter server endpoint.',
    connectRemoteJupyterInputSchema,
  ),
  defineAction(
    'connect_vscode_host',
    'Connect to the local VS Code host bridge over WebSocket.',
    connectVscodeHostInputSchema,
  ),
  defineAction(
    'disconnect_remote_jupyter',
    'Close a backend connection and clear cached notebook sessions.',
    connectionIdInputSchema,
  ),
  defineAction(
    'get_connection_status',
    'Inspect backend reachability and cached notebook sessions.',
    connectionIdInputSchema,
  ),
  defineAction(
    'list_jupyter_contents',
    'List files and directories through the active backend.',
  ...
  defineAction(
    'create_notebook',
    'Create a new notebook file in the active backend.',
    connectionIdInputSchema.extend({
      path: z.string(),
  ...
  defineAction(
    'get_notebook',
    'Fetch a notebook model including all cells and metadata.',
    connectionIdInputSchema.extend({
  ...
  defineAction(
    'list_cells',
    'List notebook cells with execution metadata and summarized outputs.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
    }),
  ),
  defineAction(
    'insert_cell',
    'Insert a cell into a notebook and save immediately.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
  ...
  defineAction(
    'update_cell',
    'Update a notebook cell and save immediately.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
      index: z.number().int().min(0),
  ...
  defineAction(
    'delete_cell',
    'Delete a notebook cell and save immediately.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
      index: z.number().int().min(0),
    }),
  ...
  defineAction(
    'move_cell',
    'Move a notebook cell and save immediately.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
  ...
  defineAction(
    'execute_code',
    "Execute code against the notebook's kernel without modifying the notebook file.",
    connectionIdInputSchema.extend({
      path: z.string().optional(),
      code: z.string(),
  ...
  defineAction(
    'run_cell',
    'Execute a single notebook cell, update outputs, and optionally save the notebook.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
      index: z.number().int().min(0),
      timeout_sec: z.number().positive().default(120),
  ...
  defineAction(
    'run_cells',
    'Execute a range of notebook cells, updating outputs and optionally stopping on the first error.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
  ...
  defineAction(
    'run_command',
    'Execute a shell command via a Python notebook kernel.',
    connectionIdInputSchema.extend({
      path: z.string().optional(),
      command: z.string(),
  ...
  defineAction(
    'upload_file',
    'Upload a local file into the backend workspace.',
    connectionIdInputSchema.extend({
      local_path: z.string(),
  ...
  defineAction(
    'download_file',
    'Download a file from the backend workspace to the local machine.',
    connectionIdInputSchema.extend({
      remote_path: z.string(),
      local_path: z.string(),
...
export function createRunCommandCode(
  command: string,
  cwd?: string,
...
export function assertPythonKernel(kernelName: string): void {
  if (!kernelName.toLowerCase().includes('python')) {
    throw new RemoteJupyterError(
      'unsupported_kernel',
      `run_command requires a Python kernel, got ${kernelName}`,
    )
  }
}
